...
await storage.setFile(fileKey, fileData, {
  mimeType: file.type,
  fileName: file.name,
  pluginId: 'chaycards/core-documents'
});
```
...
EventBus.on('documents:ready', ({ documentsService }) => {
  ...
  const OriginalCard = PluginManager.getInstance()
    .getComponent('chaycards/core-documents/FileCard');
  ...
  PluginManager.getInstance()
    .setComponent('chaycards/core-documents/FileCard', AIEnhancedCard);
});
```
...
const FileCard: React.FC<{ file: StoredFile }> = ({ file }) => {
  const documentsService = useService<DocumentsService>('chaycards/core-documents/documentsService');
